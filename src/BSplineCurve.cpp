        ...
        for(int k=2;k<=order;k++){
            for(size_t i=0;i<n+order;i++){
                ...
                if((knots[i+k-1]-knots[i])==0.f)
                    ...
                    n_ik  = i+k-1>=n+order?0.f:(t_k[t_i]-knots[i])/(knots[i+k-1]-knots[i])*b_spline_base[i];
                if((knots[i+k]-knots[i+1])==0.f)
                    n_i1k=0.f;
                else
                    n_i1k = i+k  >=n+order?0.f:(knots[i+k]-t_k[t_i])/(knots[i+k]-knots[i+1])*b_spline_base[i+1];
