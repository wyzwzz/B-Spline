        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (update_view_matrix) {
                ...
                for (auto &surface:surfaces) {
                    ...
                    surface->setupViewPos(camera->getCameraPos());
                }
            }
            if (update_projection_matrix) {
                for (auto &curve:curves)
                    curve->setupProjection(
                            glm::perspective(glm::radians(camera->getZoom()), (float) w / h, 0.1f, 50.0f));
                for (auto &surface:surfaces)
                    surface->setupProjection(
                            glm::perspective(glm::radians(camera->getZoom()), (float) w / h, 0.1f, 50.0f));
            }
        }
    };
...
int Displayer::addSurface() {
    try {
        ...
        surfaces.back()->setupViewPos(camera->getCameraPos());
    }
    catch (std::exception err) {
        std::cout << __FUNCTION__ << " error: " << err.what() << std::endl;
        return -1;
...
void Displayer::setupCurveColor(uint32_t index, std::array<float, 3> color) {
    if (index < curves.size()) {
        curves[index]->setupColor(color);
    }
}

void Displayer::setupCurvePointSize(uint32_t index, float point_size) {
    if(index<curves.size()){
        curves[index]->setupPointSize(point_size);
    }
}
